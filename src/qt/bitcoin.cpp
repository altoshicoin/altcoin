...
static const char* qt_argv = "altcoin-qt";
    ...
    Q_INIT_RESOURCE(bitcoin);
{
    ...
    // Global dialog border + gutter (inserted)
    qApp->setStyleSheet(qApp->styleSheet()
        + "QDialog { border: 1px solid #2b2b2b; padding: 6px; background-clip: padding; }");
    { QIcon appIcon(QStringLiteral(":/icons/bitcoin"));
      QApplication::setWindowIcon(appIcon); }

// Force a neutral style so QSS/palette actually shows
qApp->setStyle(QStyleFactory::create("Fusion"));

// Apply a visible dark palette immediately (so you see a change even if QSS fails)
{
    QPalette p = qApp->palette();
    p.setColor(QPalette::Window, QColor("#111111"));
    p.setColor(QPalette::WindowText, QColor("#a9a9a9"));
    p.setColor(QPalette::Base, QColor("#2a2a2a"));
    p.setColor(QPalette::AlternateBase, QColor("#242424"));
    p.setColor(QPalette::Text, QColor("#a9a9a9"));
    p.setColor(QPalette::Button, QColor("#303030"));
    p.setColor(QPalette::ButtonText, QColor("#d0d0d0"));
    p.setColor(QPalette::Highlight, QColor("#474747"));
    p.setColor(QPalette::HighlightedText, QColor("#ffffff"));
    qApp->setPalette(p);
}

// Load embedded + user QSS after the event loop starts and log bytes
QTimer::singleShot(0, []{
    QString css;
    QFile qss(":/res/css/altcoin.qss");   // embedded
    if (qss.open(QFile::ReadOnly)) css = QString::fromUtf8(qss.readAll());
    QFile userCss(QDir::homePath() + "/.altcoin/altcoin.qss");  // optional override
    if (userCss.open(QFile::ReadOnly)) css += "\n" + QString::fromUtf8(userCss.readAll());
    qInfo() << "[ALTCOIN] QSS bytes =" << css.size();
    qApp->setStyleSheet(css);
// Force a dark palette so dialogs & “base” widgets don't default to white
QPalette p = qApp->palette();
p.setColor(QPalette::Window,         QColor("#1f1f1f"));
p.setColor(QPalette::Base,           QColor("#1f1f1f"));
p.setColor(QPalette::AlternateBase,  QColor("#242424"));
p.setColor(QPalette::ToolTipBase,    QColor("#1f1f1f"));
p.setColor(QPalette::ToolTipText,    QColor("#a9a9a9"));
p.setColor(QPalette::Text,           QColor("#a9a9a9"));
p.setColor(QPalette::Button,         QColor("#2a2a2a"));
p.setColor(QPalette::ButtonText,     QColor("#a9a9a9"));
p.setColor(QPalette::WindowText,     QColor("#a9a9a9"));
qApp->setPalette(p);
});

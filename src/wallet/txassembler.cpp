    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional ALT outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes ALT and MWEB fees)
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a ALT-to-ALT transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        ...
        // If ALT-to-ALT fails, create a peg-out transaction

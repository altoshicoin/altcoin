...
userClosed(false)
{
    ...
    if (auto bg = findChild<QWidget*>("bgWidget")) {
        ...
        bg->setStyleSheet("background: rgba(0,0,0,110);");
    }
    if (auto card = findChild<QWidget*>("contentWidget")) {
        card->setAttribute(Qt::WA_StyledBackground, true);
        card->setAutoFillBackground(true);
        card->setStyleSheet("background: #d7d7d7; border: 1px solid #2b2b2b; border-radius: 8px;");
    }
for (auto lbl : this->findChildren<QLabel*>()) { lbl->setFrameShape(QFrame::NoFrame); lbl->setLineWidth(0); lbl->setAutoFillBackground(false); }
    for (auto fr  : this->findChildren<QFrame*>())  { fr->setFrameShape(QFrame::NoFrame);  fr->setLineWidth(0); }
